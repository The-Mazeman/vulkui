...
{
    Window* window = (Window*)GetProp(windowHandle, L"window");
    float deltaWidth = width - window->width;
    Window invalidA = {};
    invalidA.y = window->y;
    invalidA.height = window->height;
    Window newWindow = {};

    if(deltaWidth > 0)
    {
        newWindow.x = x + window->width;
        newWindow.y = y;
        newWindow.width = deltaWidth;
        newWindow.height = height;
        SendMessage(windowHandle, CWM_DRAW, (WPARAM)&newWindow, 0);
    }
    else
    {
        invalidA.x = window->x + width;
        invalidA.width = deltaWidth * -1.0f;
    }
    ...
    Window invalidB = {};
    invalidB.x = window->x;
    invalidB.width = window->width;
    float deltaHeight = height - window->height;
    if(deltaHeight > 0)
    {
        newWindow.x = x;
        newWindow.y = y + window->height;
        newWindow.width = width;
        newWindow.height = deltaHeight;
        SendMessage(windowHandle, CWM_DRAW, (WPARAM)&newWindow, 0);
    }
    else
    {
        invalidB.y = window->y + height;
        invalidB.height = deltaHeight * -1.0f;
    }
    ...
    uint parentIndex = 0;//window->parentIndex;
    SendMessage(parentHandle, CWM_DRAW, (WPARAM)&invalidA, parentIndex);
    SendMessage(parentHandle, CWM_DRAW, (WPARAM)&invalidB, parentIndex);
}
void moveWindow(HWND windowHandle, float x, float y, float width, float height)
{
    Window* window = (Window*)GetProp(windowHandle, L"window");
    float deltaX = x - window->x;
    float deltaY = y - window->y;
    Window invalidA = {};
    invalidA.y = window->y;
    invalidA.height = window->height;
    ...
    {
        invalidA.x = window->x;
        invalidA.width = deltaX;
    }
    else
    {
        invalidA.x = window->x + window->width + deltaX;
        invalidA.width = deltaX * -1.0f;
    }
    ...
    {
        invalidB.y = window->y;
        invalidB.height = deltaY;
    }
    else
    {
        invalidB.y = window->y + window->height + deltaY;
        invalidB.height = deltaY * -1.0f;
    }
    window->x = x;
    window->y = y;
    window->width = width;
...
{
    *window = (Window*)GetProp(windowHandle, L"window");
}
