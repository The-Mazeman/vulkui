...
layout(set = 0, binding = 6) buffer TextureVertices
{
    vec2 textureVertices[];
}; 
layout(set = 0, binding = 7) buffer TextureIndices
{
    uint textureIndices[];
}; 
layout(set = 0, binding = 8) buffer Vertices
{
    vec2 vertices[];
}; 
layout(set = 0, binding = 9) buffer EntityIndices
{
    uint entityIndices[];
}; 

layout(location = 0) out vec4 colorOutput;
layout(location = 1) out vec2 textureVertexOutput;
layout(location = 2) out uint windowTagOutput;
...
void main()
{
    ...
    vec2 vertex = vertices[(instanceIndex * 4) + vertexIndex];
    ...
    uint textureVertexIndex = textureIndices[instanceIndex];
    vec2 textureVertex = textureVertices[(textureVertexIndex * 4) + vertexIndex];
    textureVertex -= vec2(0.0, 1.0);
    textureVertex *= vec2(1.0, -1.0);
    textureVertexOutput = textureVertex;
}
